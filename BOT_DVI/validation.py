...
def validFIO(text):
    settings.cursor.execute("SELECT COUNT (*) FROM users_all WHERE first_name = '{}'".format(text))
...
def validInst(text):
    settings.cursor.execute("SELECT COUNT (*) FROM users_all WHERE inst = '{}'".format(text))
    settings.conn.commit()
    count = settings.cursor.fetchone()[0]
